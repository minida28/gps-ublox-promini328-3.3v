...
void loop()
{
    ...
    if (parseGPSCompleted)
    {
        ...
        static bool validGPSLocationFlag_old = 0;
        if (validGPSLocationFlag != validGPSLocationFlag_old)
        {
            validGPSLocationFlag_old = validGPSLocationFlag;
            ...
            if (validGPSLocationFlag)
            {
                ...
                // sprintf_P(buf, PSTR("%d, %02d:%02d:%02d"), CURRENTTIMEID, HOUR, MINUTE, SECOND);
                // Serial.println(buf);
                ...
                // u8x8.setCursor(0, 7);
                // u8x8.print(F("menuju "));
                // u8x8.print(sholatNameStr(NEXTTIMEID));
            }
        }
